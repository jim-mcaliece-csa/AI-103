(function () {
  ...
  var STORAGE_KEY = "ai103-module2-study-lab-v1";
  ...
  function savedOrderOrIdentity(order, length) {
    return isPermutation(order, length) ? order.slice() : positions(length);
  }

  // Each attempt stores its own option and row permutations. They remain stable across
  // re-renders and reloads because the presentation is persisted with the attempt.
  // Identity order is the safe fallback for an older or invalid saved presentation.
  ...
  function presentationFor(question) {
    ...
    var optionOrder = savedOrderOrIdentity(saved && saved.options, optionCount);
    var rowOrder = savedOrderOrIdentity(saved && saved.rows, rowCount);
